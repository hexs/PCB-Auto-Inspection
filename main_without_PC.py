...
class AutoInspection:
    ...
    def __init__(self):
        ...
        self.display = pg.display.set_mode((800, 480))
        self.manager = UIManager(self.display.get_size(), theme_path=self.setup_theme())
        self.right_click = RightClick(self.manager, (800, 480))
        self.setup_ui()
    ...
    def panel0_setup(self):
        model_label = UILabel(Rect((10, 5), (50, 26)), f'model:', self.manager)
        os.makedirs('data', exist_ok=True)
        model_data = os.listdir('data') + ['-']
        self.model_data_dropdown = UIDropDownMenu(model_data, '-', Rect(10, 0, 300, 30), self.manager, anchors={
            'left_target': model_label})

        self.close_button = UIButton(Rect((-40, 0, 40, 30)), f'X', self.manager, anchors={
            'top': 'top',
            'left': 'right',
            'bottom': 'top',
            'right': 'right'})

        self.t1 = UILabel(Rect((10, -24), (50, 26)), '', self.manager, anchors={
            'top': 'bottom',
            'left': 'left',
            'bottom': 'bottom',
            'right': 'left'
        })
        self.t2 = UILabel(Rect((10, -24), (170, 26)), '', self.manager, anchors={
            'top': 'bottom',
            'left': 'left',
            'bottom': 'bottom',
            'right': 'left',
        ...
        self.t3 = UILabel(Rect((10, -24), (150, 26)), '', self.manager, anchors={
            'top': 'bottom',
            'left': 'left',
            'bottom': 'bottom',
            'right': 'left',
    ...
    def panel1_setup(self):
        self.panel1_rect = Rect(0, 30, 600, 430)
    ...
    def panel2_setup(self):
        self.panel2_rect = Rect((600, 30, 200, 430))
        ...
        for event in events:
            if event.type == UI_BUTTON_PRESSED:
                ...
                if event.ui_element == self.load_button:
                    self.auto_cap_button.set_text('AutoCapture')
                    self.file_dialog = UIFileDialog(Rect(200, 50, 400, 400),
                                                    self.manager,
